...
class AsmodeusScans extends MProvider {
    ...
    for (var entry in entries) {
      ...
      list.add(
        MManga(
          ...
          name: entry.attr("title")?.substring(10),
        ),
      );
    }
    ...
    for (var entry in entries) {
      ...
      list.add(
        MManga(
          ...
          name: a.attr("title")?.substring(10),
          chapters: chapters,
        ),
      );
    }
  ...
  Future<String> getHtmlContent(String name, String url) async {
    ...
    MDocument doc = parseHtml(res.body);
    MElement pages = doc.selectFirst("#pages")!;
    if ((pages.selectFirst("img[uid]")?.outerHtml ?? "").isNotEmpty) {
      return " <h2>This chapter contains images. Please view it in WebView.</h2>";
    }
    String content = pages
        ...
        .replaceAllMapped(
          RegExp(r'uid="(\w+)"'),
          (m) => "src=\"https://cdn.meowing.org/uploads/${m[1]}\"",
        );
    return " $content";
  }
